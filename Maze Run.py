...
class Game (Scene):
	...
	def new_game(self):
		...
		self.last_row = Node(parent=self)
		self.wall_dist = 96
	...
	def update_walls(self):
		if self.timer % 64 == 0:
			...
			if random.random() < 0.65:
				...
				self.speed = min(15, self.speed+0.05)
	...
	def spawn_walls(self, y=None):
		print("Last row position: ", self.last_row.position)
		#Not allowed to use 'self' in args
		if y == None:
			y = self.size.h+72
			
		new_row = Node(position=(0, y), parent=self)
			
		ledge = SpriteNode(xWall, position=(self.wall_center-self.wall_dist, y))
		redge = SpriteNode(xWall, position=(self.wall_center+self.wall_dist, y))
		ledge.rotation=-math.pi/2
		redge.rotation=math.pi/2
		new_row.add_child(ledge)
		new_row.add_child(redge)
		self.walls.append(ledge)
		self.walls.append(redge)
		
		self.last_row = new_row
		print("New row loc: ", new_row.position)
		...
		while t <= 3:
			...
			ltile = SpriteNode(iWall, position=(lx, y))
			rtile = SpriteNode(iWall, position=(rx, y))
			
			ltile.rotation=-math.pi/2
			rtile.rotation=math.pi/2
			
			new_row.add_child(ltile)
			new_row.add_child(rtile)
